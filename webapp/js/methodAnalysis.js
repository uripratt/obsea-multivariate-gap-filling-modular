...
const MethodAnalysis = {
    ...
    methods: {
        ...
        'saits': { name: 'SAITS', color: '#2CA02C', desc: 'Self-Attention Imputation for Time Series' },
        'brits_pro': { name: 'BRITS-PRO', color: '#D55E00', desc: 'Bidirectional Recurrent Imputation (Pro)' }
    },
    ...
    async renderCaseStudy(containerId, scenarioId = 'long') {
        try {
            ...
            // 2. Dynamic Model Traces
            Object.keys(data).forEach(key => {
                // Skip non-model keys
                if (['metadata', 'timestamps', 'gap_mask', 'ground_truth', 'linear'].includes(key)) return;
                
                const modelValues = data[key];
                if (!modelValues) return;
                
                // CRITICAL SAFETY CHECK: skip if not an array (e.g. if some metadata leaked here)
                if (!Array.isArray(modelValues)) {
                    console.warn(`⚠️ Skipping non-array key: ${key}`, modelValues);
                    return;
                }

                const modelGap = modelValues.map((v, i) => isGapOrNeighbor(i) ? v : null);
                const methodConfig = this.methods[key] || { name: key.toUpperCase(), color: '#94a3b8' };
                ...
                traces.push({
                    ...
                    line: { 
                        ...
                        width: (key === 'bilstm' || key === 'saits') ? 5 : 2,
                        dash: key.includes('brits') ? 'dot' : 'solid'
                    },
                    opacity: key.includes('brits') ? 0.9 : 1.0,
                    showlegend: false
                });
            });

            // 3. Linear (always show as baseline if present)
            if (data.linear) {
                const linearGap = data.linear.map((v, i) => isGapOrNeighbor(i) ? v : null);
                traces.push({
                    x: timestamps,
                    y: linearGap,
                    mode: 'lines',
            ...
            console.log(`📊 Plotting Case Study: ${scenarioId}, total traces: ${traces.length}`, traces.map(t => t.name));
            ...
            const layout = {
                ...
                yaxis: {
                    title: 'Temperature (°C)',
                    gridcolor: '#1e3a5f',
                    autorange: true // Dynamic scale to ensure all results are visible
                },
                shapes: shapes,
                showlegend: false
            };
