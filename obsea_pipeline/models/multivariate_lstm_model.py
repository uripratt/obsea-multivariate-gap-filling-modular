...
class PhysicsInformedLoss(nn.Module):
    ...
    """
    def __init__(self, extreme_weight=2.0, smoothness_weight=0.1, max_sigma=5.0):
        super().__init__()
    ...
    def forward(self, pred, target):
        ...
        # Penalize predictions that are physically impossible (outside max_sigma bound)
        physical_bounds_loss = torch.mean(torch.relu(torch.abs(pred) - self.max_sigma))
...
class MultivariateLSTMImputer:
    ...
    def fit(
        ...
        # =====================================================================
        # CRITICAL BUG FIX (Feature Leakage):
        # We must extract the true is_observed mask BEFORE we interpolate the gaps
        train_original_mask = train_df[self.target_var].notna().astype(float)
        val_original_mask = val_df[self.target_var].notna().astype(float)
        test_original_mask = test_df[self.target_var].notna().astype(float)
        ...
        # Use the masks saved before the interpolation step!
        mask_col = f'{self.target_var}_is_observed'
        train_norm[mask_col] = train_original_mask
        val_norm[mask_col] = val_original_mask
        test_norm[mask_col] = test_original_mask
        ...
        # Loss and optimizer
        # Use Improved Physics-Informed Loss with dynamic sigma calculation
        max_abs_norm = max(abs(train_norm[self.target_var].min()), abs(train_norm[self.target_var].max()))
        dynamic_sigma = max(5.0, float(max_abs_norm) + 1.0)
        criterion = PhysicsInformedLoss(extreme_weight=2.0, max_sigma=dynamic_sigma)
        ...
        # Training loop
        logger.info(f"Starting training with Attention, Scheduler & AMP={use_amp} (Max Sigma={dynamic_sigma:.2f})...")
    ...
    def predict(self, df: pd.DataFrame) -> pd.Series:
        ...
        result = result.interpolate(method='time', limit_direction='both').bfill()
