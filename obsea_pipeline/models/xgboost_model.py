...
class XGBoostImputer:
    ...
    def _fit_single(self, df, target_var, multivariate_vars, validation_data):
        ...
        # Save climatology profile for robust inference
    ...
    def _predict_single(self, model, feat_eng, df, multivariate_vars, is_residual_mode):
        ...
        # CLIMATOLOGY BASELINE using trained profile to prevent Data Leakage
        try:
            ...
            global_mean = profile.mean() if profile is not None else 0.0
            
            mapped_values = []
            for item in idx_multi:
                if profile is not None and item in profile.index:
                    mapped_values.append(profile.loc[item])
                else:
                    mapped_values.append(global_mean)
            
            climatology = pd.Series(mapped_values, index=df_tmp.index)
        except Exception as e:
            logger.warning(f"Failed to use climatology profile: {e}")
