...
class XGBoostProImputer:
    ...
    def __init__(
        ...
        self.climatology_profiles = {} # 'fwd': profile, 'bwd': profile
    ...
    def _fit_single(self, df, target_var, multivariate_vars, validation_data):
        ...
        # 2. INTERNAL RESIDUAL LEARNING (CLIMATOLOGY)
        # We replace STL with Climatology mapping because STL cannot automatically extrapolate 
        # to future datasets (test sets), whereas climatology profiles map perfectly.
        
        valid_series = df_masked[target_var].dropna()
        group_cols_valid = [valid_series.index.dayofyear, valid_series.index.hour]
        climatology_profile = valid_series.groupby(group_cols_valid).mean()
        
        if not hasattr(self, 'climatology_profiles'):
            self.climatology_profiles = {}
        self.climatology_profiles[self._current_direction] = climatology_profile
        ...
        base_signal = climatology
        y_residual = df[target_var] - base_signal
        
        logger.info("  -> Successfully extracted Climatology base for pure residual learning.")
    ...
    def _predict_single(self, model, feat_eng, df, multivariate_vars, is_residual_mode):
        ...
        # 1. Base Reconstruction using saved Climatology components
        try:
            # Robust selection with no ambiguity
            profile = self.climatology_profiles.get(self._current_direction)
            if profile is None:
                profile = self.climatology_profiles.get('fwd')
            idx_multi = pd.MultiIndex.from_arrays([df_tmp.index.dayofyear, df_tmp.index.hour])
            global_mean = profile.mean()
            
            mapped_values = []
            for item in idx_multi:
                if item in profile.index:
                    mapped_values.append(profile.loc[item])
                else:
                    mapped_values.append(global_mean)
            
            base_series = pd.Series(mapped_values, index=df_tmp.index)
            base_series = base_series.interpolate(method='time', limit_direction='both').bfill().ffill()
        except Exception as e:
            logger.warning(f"Failed to load climatology profile, fallback to simple mean: {e}")
            global_mean = df_tmp[target].mean() if not df_tmp[target].isna().all() else 0.0
            base_series = pd.Series(global_mean, index=df_tmp.index)
    ...
    def save(self, path: str):
        joblib.dump({
            ...
            'climatology_profiles': self.climatology_profiles,
    ...
    def load(cls, path: str):
        ...
        imputer.climatology_profiles = data.get('climatology_profiles', {})
