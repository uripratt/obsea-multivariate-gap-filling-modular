...
class SAITSImputer(STLResidualMixin):
    """
    ...
    """
    def __init__(self, n_steps: int, n_features: int, n_layers: int = 2, d_model: int = 128, d_ffn: int = 256, n_heads: int = 4, dropout: float = 0.1, epochs: int = 50, batch_size: int = 16, max_gap_size: Optional[int] = None):
    ...
    def _reconstruct_from_windows(self, windows, original_len):
        ...
        for i, win in enumerate(windows):
            ...
            if np.isnan(reconstructed[start:actual_end]).all():
                reconstructed[start:actual_end] = win[:win_len]
            else:
                # Average with existing values
                reconstructed[start:actual_end] = (reconstructed[start:actual_end] * counts[start:actual_end] + win[:win_len]) / (counts[start:actual_end] + 1)
