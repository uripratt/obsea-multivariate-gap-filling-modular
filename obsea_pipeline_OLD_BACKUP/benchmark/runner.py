...
from obsea_pipeline.config.settings import CONFIG, BENCHMARK_MODELS, HARDWARE_CONFIG, GAP_CATEGORIES, BENCHMARK_CONFIG
...
from obsea_pipeline.evaluation.metrics import calculate_skill_score, spectral_error
...
def benchmark_gap_filling(df: pd.DataFrame, test_variable: str = 'TEMP', gap_categories: list = None) -> pd.DataFrame:
    ...
    def evaluate_result(interpolated, gap_mask, true_values, cat, met):
        ...
        try:
            # Baseline is climatology
            group_cols = [df.index.dayofyear, df.index.hour]
            climatology = df[test_variable].groupby(group_cols).transform('mean')
            climatology = climatology.interpolate(method='time').bfill().ffill()
            climatology_valid = climatology.loc[gap_mask][valid_mask]
            
            skill_rmse = calculate_skill_score(
                true_valid.values, predicted_valid.values, climatology_valid.values, metric='rmse'
            )
        except Exception:
            skill_rmse = np.nan
            
        try:
            spec_err = spectral_error(true_valid.values, predicted_valid.values)
        except Exception:
            spec_err = np.nan
        ...
        return {
            ...
            'Skill_RMSE': round(skill_rmse, 4) if not np.isnan(skill_rmse) else None,
            'Spectral_Error': round(spec_err, 6) if not np.isnan(spec_err) else None
        }
    
    def run_model(method, df_test, test_var, pred_vars, max_gap_size=None, df_train=None):
        try:
            ...
            elif method == 'varma': interp = interpolate_varma(df_test, df_train=df_train) # Using simplified
            elif method == 'bilstm': interp = interpolate_bilstm(df_test, test_var, predictor_vars=pred_vars, max_gap_size=max_gap_size, df_train=df_train)
            elif method == 'xgboost': interp = interpolate_xgboost(df_test, test_var, predictor_vars=pred_vars, max_gap_size=max_gap_size, df_train=df_train)
            elif method == 'xgboost_pro': interp = interpolate_xgboost_pro(df_test, test_var, predictor_vars=pred_vars, max_gap_size=max_gap_size, df_train=df_train)
            elif method == 'missforest': interp = interpolate_missforest(df_test, test_var, predictor_vars=pred_vars, df_train=df_train)
            elif method == 'saits': interp = interpolate_saits(df_test, test_var, predictor_vars=pred_vars, df_train=df_train)
            elif method == 'imputeformer': interp = interpolate_imputeformer(df_test, test_var, predictor_vars=pred_vars, max_gap_size=max_gap_size, df_train=df_train)
            elif method == 'brits': interp = interpolate_brits(df_test, test_var, predictor_vars=pred_vars, max_gap_size=max_gap_size, df_train=df_train)
            elif method == 'brits_pro': interp = interpolate_brits_pro(df_test, test_var, predictor_vars=pred_vars, df_train=df_train)
            else: interp = None
    ...
    for category in gap_categories:
        ...
        temporal_split = BENCHMARK_CONFIG.get('temporal_split', True)
        train_ratio = BENCHMARK_CONFIG.get('train_ratio', 0.7)
        
        if temporal_split:
            split_idx = int(len(df) * train_ratio)
            df_train_base = df.iloc[:split_idx].copy()
            df_test_base = df.iloc[split_idx:].copy()
            
            df_simulated_test, truth_mask_test = simulate_gaps(
                df_test_base, [test_variable], 
                missing_ratio=0.1, 
                lengths=[params['max_pts']]
            )
            
            # Combine to allow for lags and continuous context
            df_test = pd.concat([df_train_base, df_simulated_test])
            
            truth_mask = pd.concat([
                pd.DataFrame(False, index=df_train_base.index, columns=df.columns),
                truth_mask_test
            ])
            df_train_pass = df_train_base
            logger.info(f"    Split: Train={len(df_train_base)}, Test={len(df_test_base)}")
        else:
        ...
        true_values = df[test_variable]
        
        category_interps = {}
        
        for method in gpu_methods:
            if method not in methods: continue
            m_name, interp_absolute = run_model(method, df_test.copy(), test_variable, predictor_vars, max_gap_size=params['max_pts'], df_train=df_train_pass)
        ...
        if active_cpu_methods:
            cpu_outputs = Parallel(n_jobs=n_jobs_cpu, prefer="threads")(
                delayed(run_model)(method, df_test.copy(), test_variable, predictor_vars, max_gap_size=params['max_pts'], df_train=df_train_pass)
