...
def spectral_error(y_true: np.ndarray, y_pred: np.ndarray, fs: float = 1/1800) -> float:
    """
    Error Espectral basado en PSD (Power Spectral Density).
    Mide cuánto se deforma o aplana la dinámica frecuencial física original.
    fs = frecuencia de muestreo (30min = 1/1800 Hz)
    """
    valid = ~(np.isnan(y_true) | np.isnan(y_pred))
    y_true_valid = y_true[valid]
    y_pred_valid = y_pred[valid]
    
    if len(y_true_valid) < 32:
        return np.nan
        
    f_true, psd_true = welch(y_true_valid, fs=fs, nperseg=min(256, len(y_true_valid)))
    f_pred, psd_pred = welch(y_pred_valid, fs=fs, nperseg=min(256, len(y_pred_valid)))
    
    return float(np.sum(np.abs(psd_true - psd_pred)))
