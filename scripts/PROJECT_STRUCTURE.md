...
1.  **Pipeline de Procesamiento (Scripts Python)**: Se encargan de la ingesta de datos brutos, control de calidad (QC), generación de variables físicas y entrenamiento de modelos de IA para el rellenado de huecos (gap filling).
2.  **Plataforma de Visualización (Web Dashboard)**: Una interfaz profesional basada en HTML/JS que consume los resultados del pipeline para presentarlos de forma interactiva.
